...
func New(l *lexer.Lexer) *Parser {
	p := &Parser{l: l}
	//读取两个词法单元 以设置curToken 和peekToken
	p.nextToken()
	p.nextToken()
...
func (p *Parser) parseLetStatement() *ast.LetStatement {
	...
	if !p.expectPeek(token.ASSIGN) {
		p.noExpectedPeekError(token.ASSIGN, p.curToken.Type)
		return nil
	}
...
func (p *Parser) noExpectedPeekError(expect token.TokenType, actual token.TokenType) {
	msg := fmt.Sprintf("expected next token to be %s, but got %s ", expect, actual)
	p.errors = append(p.errors, msg)
}
